...
smart_ub <- function(f_name,lb,ub){
  repair <- FALSE #flag variable to see extra output text while debugging
  if(repair){
    cat("smart_ub START \n")
  }
...
#Declaring gamma as a global variable
g <<- 0.1^2
...
wt_KL <- function(w,bandit_problem,powers,i,x){
...
solve_inner_opt <- function(w,bandit_problem,powers,i){
  eval <- function(x){
    return(wt_KL(w,bandit_problem,powers,i,x)) #O(length(ai)*(-log(tolerance)) many computations
  }
  ...
  if(ans$value <= min(w[1]*K_inf_L(bandit_problem,powers,1,mui),w[i]*K_inf_U(bandit_problem,powers,i,mu1),wt_KL(w,bandit_problem,powers,i,(mu1+mui)/2))){
...
#Computing likelihood ratio
Zed <- function(emp_bandit,num_samples){
  ...
  switched_num_samples <- num_samples
  switched_num_samples[1]<- num_samples[sorted$BestEmpArm]
  switched_num_samples[sorted$BestEmpArm] <- num_samples[1]
  switched_num_samples[2]<- num_samples[sorted$SecondBestEmpArm]
  switched_num_samples[sorted$SecondBestEmpArm] <- num_samples[2]
  
  wt <- switched_num_samples/sum(switched_num_samples)
  
  ans <- g^(-max(sorted_alphas))
  for(i in 2:K){
    x <- solve_inner_opt(wt,sorted_emp_bandit,sorted_alphas,i)$optx
  ...
  return(ans*sum(switched_num_samples))    
}
...
solve_LB <- function(emp_bandit){
  ...
  x <- solve_inner_opt(opt_w,sorted_emp_bandit,sorted_alphas,2)$optx
  T_star <- (wt_KL(opt_w,sorted_emp_bandit,sorted_alphas,2,x))^(-1)
  
  if(repair){cat("\n Lower bound solving END \n")}
  return(list("Weights"=final_opt_w,"TStar"=T_star))
}
...
ExactAlgo <- function(Seed){
  set.seed(NULL)
  set.seed(Seed)
  cat("\n Seed for AL1 Algo = ",Seed,"\n")
  init_arms <- list()
  num_samples <- c()
  successes <- rep(0,K)
  #Initial exploration
  ...
  #ALGORITHM
  while(stop==FALSE){
    w_hat <- solve_LB(emp_bandit)$Weights
    zed <- Zed(emp_bandit,num_samples)
  ...
  cat("Optimal weights for current empirical distribution at l = ",l,": \n",w_hat,"\n",
      "Stopping index = ",zed,"Threshold = ",threshold,"\n")
  cat("\n Means of arms: \n",final_emp_means,"\n")
...
#The following code will run 10 sample paths of TS(A) on our chosen bandit instance
set.seed(NULL)
...
tic() #Timer starts
for(pip in seed_set[1]){
  solution <- ExactAlgo(pip)
...
toc() #Timer ends
cat("Lower bound on expected sampling complexity at ",(delta*100),"% level of confidence = ",(sol$TStar*log(1/delta)))
...
#write.csv(data,"Homogeneous output exact 5 arms.csv")