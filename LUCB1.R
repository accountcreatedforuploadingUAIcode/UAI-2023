...
#epsilon and k1 parameters for LUCB1
epsilon <<- 0
k1 <<- 5/4
...
t <- sum(num_samples)
#Tracking the best empirical mean
emp_bandit <- combine_arms(init_arms,B)
emp_means <- c()
for(i in 1:K){
  emp_means <- c(emp_means,mean_reward(emp_bandit[[i]]))
}
best_emp_arm <- sample(which(emp_means==max(emp_means)),1)
non_best_arms <- (1:K)[-best_emp_arm]
#Computing the UCB quantity for each arm in vectorised form
beta1 <- (B*g^(-alphas))*sqrt((1/(2*num_samples))*log(k1*K*t^4/delta))
ucb <- emp_means+beta1
lcb <- emp_means-beta1
#Setting the stopping rule
stop_threshold <- max(ucb[non_best_arms])-lcb[best_emp_arm]
stop_rule <- (stop_threshold>=epsilon)
tic() #Timer starts
#LUCB1 iterations
while(stop_rule){
  arm1 <- best_emp_arm
  #num_samp <- g^(-alphas[arm1])
  num_samp <- 1
  x <- sample_arm(bandit_prob,arm1,num_samp)
  u <- update_emp_bandit(emp_bandit,num_samples,x,arm1)
  emp_bandit <- u$Bandit
  num_samples <- u$NumSamples
  
  arm2 <- sample(which(ucb[non_best_arms]==max(ucb[non_best_arms])),1)
  #num_samp <- g^(-alphas[arm2])
  num_samp <- 1
  x <- sample_arm(bandit_prob,arm1,num_samp)
  u <- update_emp_bandit(emp_bandit,num_samples,x,arm2)
  emp_bandit <- u$Bandit
  num_samples <- u$NumSamples
